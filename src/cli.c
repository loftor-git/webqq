...
static void usage()
{
    fprintf(stdout, "Usage: qq [options]...\n"
            "qq: A qq client\n"
            "  -v, --version\n"
...
static void handle_new_msg(LwqqRecvMsg *recvmsg)
{
    ...
    if (msg->type == LWQQ_MS_BUDDY_MSG) {
        ...
        printf("Receive message: %s\nFrom:%s\n", charset(buf),mmsg->buddy.from->uin);
        //  lwqq_msg_send_simple(lc,LWQQ_MS_BUDDY_MSG, mmsg->buddy.from->uin, charset(buf));
   ...
   // lwqq_msg_send_simple(lc,LWQQ_MS_BUDDY_MSG, msg->uin, argv[2]);
...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "vhu:p:",
        ...
        switch (c) {
        case 'v':
            printf("cli version %s, Copyright (c) 2014 "
                   "mathslinux\n", LWQQ_CLI_VERSION);
            exit(0);
        case 'h':
            usage();
    ...
    lc->action = &act;
    if (!lc) {
        lwqq_log(LOG_NOTICE, "Create qq client failed\n");
        return -1;
    }
